...
testExp(Ex, Text, Vars, [Var, not(Val) | Rest]) :- !,
    (   getVal(Var, Vars, V), !,
        (   V \== Val, !, testExp(Ex, Text, Vars, Rest) ;
            swritef(M, 'Interogarea %w leaga %w la %w dar nu ar fi trebuit ca valoarea sa fie %w.',
...
testSet(Ex, Text, TypeText, SetG, SetE) :-
    ...
    (   SetGSorted == SetESorted, ! ;
        ...
        (   TooMuch == [], TooLittle == [], !,
            M1 = 'vezi duplicate'
        ;   swritef(M1, 'elementele %w sunt in plus, elementele %w lipsesc', [TooMuch, TooLittle])
        ),
        swritef(M,

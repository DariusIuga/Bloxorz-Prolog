...
tt(s0, [
       ...
       exp('ttSet0(S), get_cell(S, (0, 0), X), get_cell(S, (0, 1), Y)',
          ['X', tile, 'Y', target]),
       % f
   0.5, uck((ttSet0(S), get_cell(S, (0, 2), _))), % celula nu există
   0.5, uck((ttSet0(S), get_cell(S, (3, 0), _))), % celula nu există
       ...
       exp('ttSet0(S), move(S, r, S1), get_b_pos(S1, P)',
          [set('P', [(1, 0), (2, 0)])]),
       % k
   0.5, uck((ttSet0(S), move(S, d, _))), % blocul cade
   0.5, uck((ttSet0(S), move(S, l, _))) % blocul cade
   ]).

tt(s1, [
       % a
       exp('ttSet(1, S), get_cell(S, (0, 0), X)', ['X', tile]),
 0.25, uck((ttSet(1, S), get_cell(S, (9, 2), _))), % celula este goală
 0.25, uck((ttSet(1, S), get_cell(S, (9, 5), _))),
 0.25, uck((ttSet(1, S), get_cell(S, (-1, 2), _))),
 0.25, uck((ttSet(1, S), get_cell(S, (3, 7), _))),
       % f
    ...
    2, exp('ttSet(1, S), ttSeq(S, [r,r,d,l,l,l,u], S1), get_b_pos(S1, P)',
       ...
       uck((ttSet(1, S), ttSeq(S, [d,r,r,r,r,r,r,r,r], _))),
...
tt(s3, [
       ...
       % f
        ...
        uck((ttSet(3, S), ttSeq(S, [r,u,u,l,d,r,r,u,r], _))),

        % secventa de mutari nu schimba tile-urile
     2, ech("ttSet(3, S), ttSeq(S, [u,r,r,d,u,u,r,r,r,r], S1), between(1, 8, X)",
            ["get_cell(S, (X, 2), T), T == tile", "get_cell(S1, (X, 2), T), T == tile"])
    ]).

tt(s4, [
       % a
        uck((ttSet(4, S), get_cell(S, (0, 0), _))),
   ...
   0.3, exp('ttSet(4, S), get_cell(S, (12, 7), T)', [val('T'), 'T', not(fragile)]),
       % f
        ...
        uck((ttSet(4, S), ttSeq(S, [u,l,u,r,r,u,r,r,r,r,r,r,d,r,d,d,d,l], _))),
...
tt(s6, [
      % a
       ...
       ech("ttSet(6, S), between(0, 4, X), get_cell(S, (X, 0), T)", ['T', tile]), % verifica ca primele 5 celule de pe primul rand sunt placi
  ...
  1.5, exp('ttSet(6, S), ttSeq(S, [r,r,r,d,d,r,d,d,r,d,r,u,l,l,l,u,u,l,u,u,u,r], S1), get_b_pos(S1, P)',
       ...
       ech("ttSet(6, S), ttSeq(S, [d,d,r,r,r,r,r], S1), between(1, 8, X)", % verifica daca secventa nu schimba placile
